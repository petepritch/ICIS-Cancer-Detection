...
class TumorPreprocessor:
    ...
    def get_dataloaders(self, test_size=0.2, random_state=42):
        ...
        #TODO stratify across participants
        ...
        #putting labeling_mode into the weighted sampler
        high_weight = 1.0
        low_weight = self.undersamplingweight
        sample_weights = np.full(len(train_df), low_weight)
        
        #standard condition, only actually malignant tumors get oversampled
        originally_malignant_mask = (train_df['target']==1)
        sample_weights[originally_malignant_mask] = high_weight
        
        #indeterminate condition
        if self.labeling_mode == "indeterminate_as_malignant":
          #find benign samples that meet criteria
          is_benign_mask = (train_df['target'] == 0)
          #is_indeterminate_mask = (train_df['iddx_1'] == 'Indterminate')
          has_iddx2_mask = pd.notna(train_df['iddx_2'])
          #combine benign and (indterminate OR has_iddx2)
          #relabel_mask = is_benign_mask & (is_indeterminate_mask | has_iddx2_mask)
