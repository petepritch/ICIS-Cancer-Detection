      ...
      def __init__(self, pipeline, data, target = None):
        ...
        if isinstance(target, pd.DataFrame):
          ...
          if target is None:
            ...
            raise TypeError("target of `PipelineWrapper` must be one of pd.DataFrame/pd.Series/None !")

        if isinstance(self.data, pd.DataFrame):
          self.index = self.data.index
          self.columns = self.data.columns
        else:
          raise TypeError("Input data of `PipelineWrapper` should be `pandas.DataFrame` object!")
        ...
        for name, transformer in self.pipeline.steps:
          ...
          if isinstance(transformer, ColumnTransformer): # ColumnTransformer
            for subname, subtransformer, features in transformer.transformers:
              print("*" * 20)
              print(f"Implement subtransformer {subname} of ColumnTransformer")
              if subname == "target_encoder":
                self.data[features] = subtransformer.fit_transform(self.data[features], self.target)
              else:
                self.data[features] = subtransformer.fit_transform(self.data[features])
              print("*" * 20)
            ...
            raise TypeError(f"Procedure for {name} undefined in `PipelineWrapper`!")
          print("-" * 20)
        
        return self.data

    def __init__(self, csv_file_path, pipeline, target_column = None, id_column = None,
                 columns_to_drop = None, transform = None, dtype = DTYPE):
        ...
        self.dtype = dtype
        self.transform = transform if transform else transforms.Lambda(lambda x: torch.tensor(x, dtype = dtype))

        # Define columns to drop (id_column + target_column + additional drops)
        columns_to_remove = [id_column] + ([target_column] if target_column else []) + (columns_to_drop if columns_to_drop else [])
        columns_to_remove = list(set(columns_to_remove))  # Remove duplicates

        # Extract features (drop specified columns)
        self.features = self.data.drop(columns = columns_to_remove) # Remove unused features
        self.pipeline = self.PipelineWrapper(pipeline, self.features, self.data[target_column]) # Wrap pipeline
